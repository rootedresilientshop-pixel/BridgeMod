"""Async SQLite database manager for DreamCraft: Legacies v2."""
...
class DatabaseManager:
    ...
    def __init__(self, database_path: str | Path) -> None:
        ...
        self.schema_path = Path(__file__).resolve().parent / "data" / "schema.sql"
        configured_journal_mode = settings.sqlite_journal_mode.upper()
        self.sqlite_journal_mode = (
            configured_journal_mode if configured_journal_mode in _ALLOWED_JOURNAL_MODES else "WAL"
        )
        self.sqlite_busy_timeout_ms = max(0, settings.sqlite_busy_timeout_ms)
    ...
    async def get_connection(self) -> AsyncIterator[aiosqlite.Connection]:
        ...
        await connection.execute(f"PRAGMA journal_mode = {self.sqlite_journal_mode}")
        await connection.execute(f"PRAGMA busy_timeout = {self.sqlite_busy_timeout_ms}")
    ...
    async def get_character(self, character_id: int) -> dict[str, Any] | None:
        ...
        return await self.fetch_one("SELECT * FROM characters WHERE id = ?", (character_id,))
    ...
    async def get_characters(self, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
    ...
    async def get_characters_at_location(self, location_id: int) -> list[dict[str, Any]]:
        ...
        return await self.fetch_all("SELECT * FROM characters WHERE location_id = ?", (location_id,))
    ...
    async def get_events_for_day(self, day: int) -> list[dict[str, Any]]:
        ...
        return await self.fetch_all("SELECT * FROM events WHERE day = ? ORDER BY tick, id", (day,))
